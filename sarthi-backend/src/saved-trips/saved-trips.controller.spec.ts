...
describe('SavedTripsController', () => {
  ...
  it('share calls service.enableSharing', async () => {
    mockService.enableSharing.mockResolvedValue({
      shareToken: 'abc',
      url: '/shared-trips/abc',
    });
    const result = await controller.share('trip-1', mockRequest as any);
    expect(mockService.enableSharing).toHaveBeenCalledWith(
      'trip-1',
      mockRequest.user,
    );
  ...
  it('unshare calls service.disableSharing', async () => {
    ...
    expect(mockService.disableSharing).toHaveBeenCalledWith(
      'trip-1',
      mockRequest.user,
    );
  });
