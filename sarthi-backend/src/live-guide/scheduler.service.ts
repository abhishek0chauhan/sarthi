...
export class SchedulerService {
  ...
  async tick(istHour: number, istMinute: number): Promise<void> {
    ...
    for (const session of sessions) {
      try {
        if (istHour === 7) {
          const lastBriefing = session.lastBriefingAt
            ? new Date(session.lastBriefingAt).toISOString().split('T')[0]
            : null;
        ...
        if (istHour === 8 && istMinute === 0) {
          const lastBreakfast = session.lastBreakfastAt
            ? new Date(session.lastBreakfastAt).toISOString().split('T')[0]
            : null;
          if (lastBreakfast !== today) {
            await this.liveGuideService.sendMealNudge(
              session as any,
              'breakfast',
            );
          }
        ...
        if (istHour === 13 && istMinute === 0) {
          const lastLunch = session.lastLunchAt
            ? new Date(session.lastLunchAt).toISOString().split('T')[0]
            : null;
          if (lastLunch !== today) {
        ...
        if (istHour === 19 && istMinute === 0) {
          const lastDinner = session.lastDinnerAt
            ? new Date(session.lastDinnerAt).toISOString().split('T')[0]
            : null;
          if (lastDinner !== today) {
        ...
        this.logger.warn(
          `Scheduler tick error for session ${session.id}: ${(err as Error).message}`,
        );
      }
    }
  }
