...
const TREK_INTENT_KEYWORDS =
  /\b(trek|treks|trekking|hike|hiking|summit|mountaineering|base\s*camp|high\s*altitude)\b/i;
...
export class TrekService {
  ...
  isTrekkingIntent(experienceTypes: string[], freeText: string): boolean {
    const hasTypeMatch = experienceTypes.some((t) =>
      TREK_INTENT_EXPERIENCE_TYPES.has(t.toLowerCase()),
    );
  ...
  filterForSearch(filter: SearchTrekFilter): Trek[] {
    ...
    const tripDays = Math.ceil((to.getTime() - from.getTime()) / 86400000) + 1;

    const needsEasier =
      (filter.age && filter.age >= 55) ||
